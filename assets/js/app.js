...
function xScale(dataSet, chosenXAxis) {
    ...
    var xLinearScale = d3.scaleLinear()
        .domain([d3.min(dataSet, d => d[chosenXAxis]) * 0.1,
        d3.max(dataSet, d => d[chosenXAxis]) * 1.9
        ])
        .range([0, chartWidth]);
...
function yScale(dataSet, chosenYAxis) {
    ...
    var yLinearScale = d3.scaleLinear()
        .domain([d3.min(dataSet, d => d[chosenYAxis]) * 0.1,
        d3.max(dataSet, d => d[chosenYAxis]) * 1.9
        ])
        .range([chartHeight, 0]);
...
d3.csv("data.csv").then(function (dataSet) {
    ...
    var circlesGroup = chartGroup.selectAll("circle")
        ...
        .attr("cy", d => xLinearScale(d[chosenYAxis]))
    ...
    ylabelsGroup.selectAll("text")
        .on("click", function () {
            ...
            if (value_y !== chosenYAxis) {
                ...
                // updates circles with new x values
                circlesGroup = renderYCircles(circlesGroup, xLinearScale, chosenXAxis);
